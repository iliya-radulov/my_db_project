...
from stage_two.alloy.alloy_screening_v2 import screen_composition
from stage_two.alloy.alloy_calculator_v2 import ATOMIC_WEIGHTS
from stage_two.lookup.lookup_common_v2 import (
    from_mp_results, from_oqmd_results, from_alexandria_results,
    dedup_by_formula, filter_by_distance
)
from stage_two.tools.plot_v2 import plot_xrd, plot_vsm, plot_sem
...
class AlloyLabApp(ctk.CTk):
    ...
    def viewer_load_plot(self, char_id):
        ...
        try:
            ...
            if char_type == 'XRD':
                ...
                # Don't create a canvas here - plot_sem already did it!
                if error:
                ...
                return  # ← IMPORTANT: Exit here because plot_sem handled everything
            else:
                self.viewer_plot_label.pack()
                self.viewer_plot_label.configure(text=f"Unsupported file type: {char_type}")
                return
            
            # XRD and VSM: Create canvas here
            if error:
                self.viewer_plot_label.pack()
                self.viewer_plot_label.configure(text=f"Error: {error}")
                return
    ...
    def validate_formula(self, event=None):
        ...
        try:
            from stage_two.alloy.alloy_calculator_v2 import parse_composition_input
    ...
    def run_calculation(self):
        ...
        try:
            ...
            from stage_two.alloy.alloy_calculator_v2 import (
            ...
            if screening is not None:
                ...
                synth = screening.get('synthesis_feasibility')
                if synth:
                    status_icon = {'ok': '✅', 'caution': '⚠️', 'blocked': '🚫', 'unknown': '❓'}.get(synth['status'], '')
                    output.append(f"  {status_icon} Synthesis [{synth['status']}]: {synth['message']}")
                    if synth['suggested_routes']:
                        output.append(f"     Suggested routes: {', '.join(synth['suggested_routes'])}")
            else:
    ...
    def submit_to_db(self):
        ...
        try:
            ...
            if screening is not None:
                synth = screening.get('synthesis_feasibility')
                if synth and synth['status'] == 'blocked':
                    proceed = messagebox.askyesno(
                        "Synthesis Feasibility Warning",
                        f"{synth['message']}\n\n"
                        f"Suggested routes: {', '.join(synth['suggested_routes'])}\n\n"
                        f"This is advisory only (composition-only check, no DFT/literature "
                        f"lookup) -- save this sample anyway?"
                    )
                    if not proceed:
                        self.status_label.configure(text="Cancelled (synthesis feasibility)")
                        return
