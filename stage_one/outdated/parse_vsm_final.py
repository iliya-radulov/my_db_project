#!/usr/bin/env python3
"""
VSM Data Parser for PPMS .dat files - Final version
...
def parse_vsm_file(file_path):
    ...
    # Parse data rows - only keep Temperature, Field, Moment, Error
    fields = []
    moments = []
    temps = []
    errors = []
...
def get_vsm_summary(file_path):
    ...
    summary = {
        ...
        'coercivity': data['hc'],
        'mass': data.get('mass'),
        'mass_source': 'filename' if data.get('mass') else 'header'
    }
