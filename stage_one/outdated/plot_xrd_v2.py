#!/usr/bin/env python3
"""
XRD and VSM Plotting Module - with fixed hysteresis loop markers
"""
...
def plot_vsm(file_path, figure=None):
    ...
    if hc:
        # Vertical red line at Hc (from y-axis min to max)
        y_min, y_max = ax.get_ylim()
        ax.plot([hc, hc], [y_min, y_max], 'r--', alpha=0.7, linewidth=1.5, label=f'Hc = {hc:.1f} Oe')
        ax.plot([-hc, -hc], [y_min, y_max], 'r--', alpha=0.7, linewidth=1.5)
        # Add Hc label at the crossing point
        ax.text(hc, 0, f'Hc={hc:.1f}', fontsize=8, ha='left', va='bottom', color='red')
        ax.text(-hc, 0, f'Hc={hc:.1f}', fontsize=8, ha='right', va='bottom', color='red')
    
    # --- Mr: green line at zero field (y-axis crossing) ---
    if mr:
        # Horizontal green line at Mr (from x-axis min to max)
        x_min, x_max = ax.get_xlim()
        ax.plot([x_min, x_max], [mr, mr], 'g--', alpha=0.7, linewidth=1.5, label=f'Mr = {mr:.4f} emu')
        ax.plot([x_min, x_max], [-mr, -mr], 'g--', alpha=0.7, linewidth=1.5)
        # Add Mr label at the crossing point
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python plot_xrd_v2.py <file_path>")
        sys.exit(1)
