...
from plot_xrd import plot_xrd, plot_vsm
                ...
                if char_type == 'XRD' and ext == '.xy':
                    try:
                        ...
                        if xrd_result['success']:
                            self.import_log.insert("end", f"  📐 XRD: a = {xrd_result.get('lattice_a', 'N/A'):.4f} Å\n")
                    except Exception as e:
                        self.import_log.insert("end", f"  ⚠️ XRD analysis failed: {str(e)}\n")
                
                # Parse VSM files automatically
                if char_type in ['VSM', 'MH'] and ext == '.dat':
                    try:
                        from vsm_integration import import_vsm_file
                        vsm_result = import_vsm_file(file_path, sample_id, db)
                        if vsm_result['success']:
                            self.import_log.insert("end", f"  📊 VSM: Ms = {vsm_result.get('ms', 0):.4f} emu, Hc = {vsm_result.get('hc', 0):.1f} Oe\n")
                    except Exception as e:
