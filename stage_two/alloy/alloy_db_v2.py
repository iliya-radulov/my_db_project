...
class AlloyDB:
    ...
    def add_compositions(self, sample_db_id: int, elements: list,
                         composition_type: str = 'aimed') -> None:
        """
        Writes one row per element to the compositions table.

        elements: list of ElementComponent (from alloy_calculator_v2) --
            each must have .symbol, .at_pct, .wt_pct.
        composition_type: 'aimed' | 'measured' | 'reference'

        Replaces any existing rows of the same type for this sample,
        so re-entering a composition cleanly overwrites the old one
        rather than appending duplicates.
        """
        self.cursor.execute(
            "DELETE FROM compositions WHERE sample_id = %s AND composition_type = %s",
            (sample_db_id, composition_type)
        )
        for e in elements:
            self.cursor.execute(
                """INSERT INTO compositions
                       (sample_id, element, weight_percent,
                        atomic_percent, composition_type)
                   VALUES (%s, %s, %s, %s, %s)""",
                (sample_db_id, e.symbol,
                 round(float(e.wt_pct), 4),
                 round(float(e.at_pct), 4),
                 composition_type)
            )
        self.commit()
