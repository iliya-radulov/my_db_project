...
class XRDAnalyzerApp(ctk.CTk):
    ...
    def save_to_db(self):
        ...
        try:
            ...
            # Look up sample integer id from samples table by string sample_id.
            # characterization.sample_id can be NULL if import didn't link properly,
            # so we ask the operator directly -- they know which sample this is.
            sample_str = ctk.CTkInputDialog(
                text="Enter sample ID (e.g. RP1a):",
                title="Which sample is this file for?"
            ).get_input()
            if not sample_str:
                conn.close()
                return
            cur.execute(
                f"SELECT id FROM {schema}.samples WHERE sample_id = %s",
                (sample_str.strip(),)
            )
            sample_row = cur.fetchone()
            if not sample_row:
                messagebox.showerror("Sample not found",
                    f"Sample '{sample_str}' not found in database.")
                conn.close()
                return
            sample_db_id = int(sample_row["id"])
            ...
            for r in peaks_rows:
                ...
                sql = 'INSERT INTO ' + schema + '.xrd_peaks (' + col_names + ') VALUES (' + placeholders + ')'
                cur2.execute(sql, r)
            cols = list(features_row.keys())
            col_names = ', '.join(cols)
            placeholders = ', '.join(['%(' + c + ')s' for c in cols])
            sql = 'INSERT INTO ' + schema + '.xrd_features (' + col_names + ') VALUES (' + placeholders + ')'
            cur2.execute(sql, features_row)
            conn.commit()
            cur2.close()
            cur.close()
            conn.close()
