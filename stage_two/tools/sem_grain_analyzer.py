...
def analyze_sem_grains(file_path, sem_metadata=None, canny1=None, canny2=None,
    ...
    if sem_metadata is None:
        # Use universal parser — handles Zeiss/JEOL/Tescan
        # and returns pixel_size_nm already as a float
        from sem_metadata_universal import parse_sem_metadata_universal
        sem_metadata = parse_sem_metadata_universal(file_path)

    # sem_metadata_universal returns pixel_size_nm as float;
    # parse_sem_v2 returns it as a string like '11.09 nm' -- handle both.
    _px = sem_metadata.get('pixel_size_nm')
    if isinstance(_px, (int, float)):
        pixel_size_nm = float(_px)
    else:
        pixel_size_nm = parse_pixel_size(_px)
