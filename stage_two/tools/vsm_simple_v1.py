#!/usr/bin/env python3
"""
Simple VSM Parser - Reads all valid rows
"""
...
def parse_vsm_simple(file_path):
    """Simple parser that reads all valid rows"""
    
    with open(file_path, 'r', encoding='latin-1') as f:
        lines = f.readlines()
    
    # Find the header (row with 'Magnetic Field')
    header_idx = None
    ...
    for i in range(header_idx + 1, len(lines)):
        ...
        try:
            field = float(parts[3].strip())
            moment = float(parts[4].strip())
            fields.append(field)
...
def analyze(file_path):
    data = parse_vsm_simple(file_path)
    if 'error' in data:
        print(f"❌ {data['error']}")
        return
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("Usage: python vsm_simple.py <file.dat>")
