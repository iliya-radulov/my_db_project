#!/usr/bin/env python3
"""
Alloy Lab Desktop App - Enhanced
- Persistent sample ID counter
- Manual entry in import tab
"""
...
class AlloyLabApp(ctk.CTk):
    ...
    def setup_new_entry_tab(self):
        ...
        self.auto_generate_id()  # Initial ID
    ...
    # ============================================
    # Tab 2: Import Files (Enhanced with Manual Entry)
    # ============================================
    ...
    def refresh_files(self):
        ...
        for f in files:
            ...
            # Sample dropdown + manual entry option
            sample_options = [""] + self.get_all_sample_ids() + ["[Enter Manual]"]
    ...
    def apply_manual_to_selected(self):
        """Apply manually entered sample ID to all selected files"""
        ...
        if not re.match(r'^[A-Za-z0-9\-_]+$', manual_id):
            if not messagebox.askyesno("Warning", f"Sample ID '{manual_id}' contains special characters.\nContinue?"):
                return
        ...
        self.import_log.insert("end", f"✏️ Applied manual ID '{manual_id}' to {count} selected file(s)\n")
        self.import_log.see("end")
