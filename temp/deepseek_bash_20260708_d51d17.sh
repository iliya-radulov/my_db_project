cat > alloy_desktop.py << 'ENDOFFILE'
#!/usr/bin/env python3
"""
...
class AlloyLabApp(ctk.CTk):
    ...
    def submit_to_db(self):
        ...
        try:
            ...
            sample_id = input("Enter Sample ID [NEW-{date_str}-001]: ") or f"NEW-{date_str}-001"
            ...
            # This reuses the lookup logic from alloy_entry_full
            api_key = get_api_key()
            if api_key:
                ...
                for match in deduped:
                    db.add_literature_check(
                        sample_db_id=db.cursor.lastrowid,
